...
public class Sign1COSEObject extends SignCommon {
  ...
  protected void DecodeFromCBORObject(CBORObject messageObject)
    ...
    if (messageObject.get(2).HasTag(EInteger.FromInt32(24))) {
      ...
      if (messageObject.get(2).getType() == CBORType.ByteString) rgbContent =
        messageObject.get(2).GetByteString();
      else if (!messageObject.get(2).isNull()) throw new CoseException(
        "Invalid Sign1 structure"
      );
    }

    if (messageObject.get(3).getType() == CBORType.ByteString) rgbSignature =
  ...
  protected CBORObject EncodeCBORObject() throws CoseException {
    if (rgbSignature == null) throw new CoseException(
