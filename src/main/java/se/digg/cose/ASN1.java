...
public class ASN1 {
  ...
  public static ArrayList<TagValue> DecodeSubjectPublicKeyInfo(byte[] encoding)
    ...
    if (tvl.get(0).list.get(0).tag != 6) throw new CoseException(
      "Invalid SPKI"
    );
  ...
  public static TagValue DecodeCompound(int offset, byte[] encoding)
    ...
    if ((encoding[offset] & 0x20) != 0x20) throw new CoseException(
      "Invalid structure"
    );
    int[] l = DecodeLength(offset + 1, encoding);
    int sequenceLength = l[1];
    if (offset + sequenceLength > encoding.length) throw new CoseException(
      "Invalid sequence"
    );
    offset += l[0] + 1;
  ...
  public static TagValue DecodeSimple(int offset, byte[] encoding)
    ...
    int sequenceLength = l[1];
    if (offset + 2 + sequenceLength != encoding.length) throw new CoseException(
      "Invalid sequence"
    );
  ...
  public static ArrayList<TagValue> DecodePKCS8Structure(byte[] encodedData)
    ...
    if (retValue.get(1).tag != 0x30) throw new CoseException(
      "Invalid PKCS8 structure"
    );
    if (retValue.get(1).list.isEmpty() || retValue.get(1).list.size() > 2) {
      throw new CoseException("Invalid PKCS8 structure");
    }
    if (retValue.get(1).list.get(0).tag != 6) throw new CoseException(
      "Invalid PKCS8 structure"
    );
    //  Dont check the next item as it is an ANY

    if (retValue.get(2).tag != 4) throw new CoseException(
      "Invalid PKCS8 structure"
    );
  ...
  public static ArrayList<TagValue> DecodePKCS8RSA(ArrayList<TagValue> pkcs8)
    ...
    if (pkdl.size() < 8 || pkdl.size() > 11) throw new CoseException(
    ...
    for (TagValue tagValue : pkd.list) {
      if (tagValue.tag != IntegerTag) throw new CoseException(
        "Invalid RSAPrivateKey"
      );
    }
  ...
  public static ArrayList<TagValue> DecodePKCS8EC(ArrayList<TagValue> pkcs8)
    ...
    if (pkdl.size() < 2 || pkdl.size() > 4) throw new CoseException(
    ...
    if (pkdl.size() > 2) {
      if ((pkdl.get(2).tag & 0xff) != 0xA0) {
        ...
        if (
          pkdl.size() == 4 && (pkdl.get(3).tag & 0xff) != 0xa1
        ) throw new CoseException("Invalid ECPrivateKey");
      }
    }
